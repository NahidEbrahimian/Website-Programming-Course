...
function check_game(){
    for(let  i = 0; i < 3; i++){
        ...
        if(flags[i][0] == "O" && flags[i][1] == "O" && flags[i][2] == "O"){
            score_o_counter++;
            score_o.innerHTML = score_o_counter;
            o_winner = 1;
        }
        if(flags[0][i] == "X" && flags[1][i] == "X" && flags[2][i] == "X"){
            score_x_counter++;
            score_x.innerHTML = score_x_counter;
            x_winner = 1;

        }
        if(flags[0][i] == "O" && flags[1][i] == "O" && flags[2][i] == "O"){
            score_o_counter++;
            score_o.innerHTML = score_o_counter;
            o_winner = 1;
...
function display_winner(){
        if(x_winner == 1){
            ...
            // winner_background.classList.add("winner")
        }
        else if(o_winner == 1){
