        ...
        <nav class="navbar navbar-expand-lg navbar-dark mb-2 rounded ">
            <div class="container-fluid">
                <?php if ($_SESSION["login_status"] == null || $_SESSION["login_status"] == false) : ?>
                    <a class="nav-link text-dark fs-6" href="index.php"><b>خدمات پزشکی </b></a>
                <?php endif; ?>
                ...
                <div class="collapse navbar-collapse" id="navbarColor01">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0 col-lg-auto me-lg-auto justify-content-center ">
                        ...
                        <?php if ($_SESSION["login_status"] != null && $_SESSION["login_status"] == true) : ?>
                            ...
                            if($_SESSION["user-login"]=="patient")
                            {
                                $user = $db->query("SELECT * FROM patients WHERE id = $user_id")->fetch_assoc();
                            }
                            else
                            {
                                $user = $db->query("SELECT * FROM doctors WHERE id = $user_id")->fetch_assoc();
                                $specialty_id = $user["specialty_id"];
                                $specialty = $db->query("SELECT * FROM specialties WHERE id=$specialty_id")->fetch_assoc();
                            }
                            ?>
